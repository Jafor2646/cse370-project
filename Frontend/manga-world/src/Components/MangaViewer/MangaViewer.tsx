        ...
        <div>
            {stars.map((star) => (
                <span
                    ...
                    className={`cursor-pointer ${star <= value ? 'text-yellow-500' : 'text-gray-300'} text-5xl`}
                >
                    &#9733;
                </span>
            ))}
...
function MangaViewer() {
    ...
    useEffect(() => {
        if(selectedManga){
            ...
            if(currentlySelected === "volumes"){
                ...
                RateController.getAllRating(currentRatingPage-1, selectedManga).then((res) => {
                    setRates(res.data);
                });
            }
        }
    ...
    useEffect(() => {
        if (selectedOption && selectedOption !== hasWishlist && selectedOption !== "interested" && authorised ==='true' && userId && selectedManga){
            UserController.getUserById(userId).then((res) => {
                let user:User = res.data;
                let tempWishlist = {
                    user: res.data,
        ...
        if(selectedOption === "interested" && hasWishlist!=="interested" && authorised ==='true' && userId && selectedManga){
            UserController.getUserById(userId).then((res) => {
                let user:User = res.data;
                let tempWishlist = {
                    user: res.data,
            ...
            <div className="bg-black bg-opacity-90 min-h-screen">
                    ...
                    <div className="fixed z-10 inset-0 overflow-y-auto flex items-center justify-center backdrop-filter backdrop-blur-sm">
                        <div className="bg-black border-2 border-gray-500 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:max-w-md sm:w-full">
                            <div className="bg-black px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                                <h3 className="text-lg leading-6 font-medium text-white">Leave a Review</h3>
                                <div className="mt-4">
                    ...
                    {selectedManga && (
                        <div className="flex justify-between gap-x-10">
                            <div className="m-2 ms-4 text-white max-w-7xl">
                                ...
                                <div className="mt-2">
                                    Average Rating:{" "}
                                    <span className="font-bold" style={{ color: getReviewPointColor(averageRating) }}>
                                        {averageRating + " "}
                                    </span>
                                    / 5
                                    <span onClick={() => {setCurrentlySelected("reviews")}} className="text-blue-500 hover:cursor-pointer hover:underline">
                                        {" "}[{totalRating} Reviews]
                                    </span>
                                </div>
                            ...
                            {authorised === "true" && (
                                <div className="mt-4">
                                    ...
                                    >
                                        <option value="interested">{hasWishlist !== "interested" ? "Remove Selection" : "Interested?"}</option>
                                        <option value="ongoing">{hasWishlist === "ongoing" ? "Ongoing" : "Add to Ongoing?"}</option>
                                        <option value="wishlist">{hasWishlist === "wishlist" ? "Wish Listed" : "Add to Wishlist?"}</option>
                                        <option value="alreadyread">{hasWishlist === "alreadyread" ? "Finished" : "Add to Completed?"}</option>
                                    </select>
                                </div>
                            )}
                ...
                <div className="m-4 mb-0">
                    <div className="flex justify-center">
                        <div onClick={() => {setCurrentlySelected("volumes")}} className={` ${currentlySelected === "volumes" ? "bg-gray-900 hover:bg-gray-900 hover:pointer-events-none" : "bg-gray-700 hover:bg-gray-800 hover:cursor-pointer hover:scale-110 transition-transform duration-300 ease-in-out"} text-2xl p-4 rounded font-extrabold text-center text-white me-4`}>
                            Volumes
                        </div>
                        <div onClick={() => {setCurrentlySelected("reviews")}} className={` ${currentlySelected === "reviews" ? "bg-gray-900 hover:bg-gray-900 hover:pointer-events-none" : "bg-gray-700 hover:bg-gray-800 hover:cursor-pointer hover:scale-110 transition-transform duration-300 ease-in-out"} text-2xl p-4 rounded font-extrabold text-center text-white me-4`}>
                            Reviews
                        </div>
                    </div>
                    ...
                    {currentlySelected === "reviews" && (
                        <>
                            <div className="ms-32 me-32">
                                {rates.map((rate) => (
                                    <div className="flex flex-col bg-black bg-opacity-25 text-white border-2 border-gray-500 rounded-lg p-4 mt-2" key={rate.rid}>
                                        <div className="flex justify-between">
                                            <div className="font-bold">
                                                {rate.user.name} {"   ("}
                                                <span className="font-bold" style={{ color: getReviewPointColor(rate.rvalue) }}>
                                                    {rate.rvalue + ""}
                                                </span>
                            ...
                            <Pagination totalItems={totalRating} itemsPerPage={10} onPageChange={onPageChange}/>
                        </>
