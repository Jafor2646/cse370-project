...
function NewManga() {
    ...
    const handleAuthorChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const tempSelectedAuthor = authors.find((author) => author.aid === parseInt(e.target.value));
    ...
    const handleCategoriesChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
        const tempSelectedCategory = categories.find((category) => category.cid === parseInt(e.target.value));

        if (tempSelectedCategory && !addedCategories.includes(tempSelectedCategory)) {
            setSelectedCategory(tempSelectedCategory.cid.toString());
        }
    };

    const handleAddAuthor = () => {
        let tempSelectedAuthor = authors.find((author) => author.aid === parseInt(selectedAuthor));
        if (tempSelectedAuthor && selectedAuthor && !addedAuthors.includes(tempSelectedAuthor)) {
    ...
    const handleAddCategory = () => {
        let tempSelectedCategory = categories.find((category) => category.cid === parseInt(selectedCategory));
        if (tempSelectedCategory && selectedCategory && !addedCategories.includes(tempSelectedCategory)) {
    ...
    const handleSubmit = () => {
        ...
        MangaController.addNewManga(tempManga).then((resManga) => {
            ...
            if(picture){
                ...
                MangaPictureController.addNewPicture(tempMangaPicture).then((resPicture) => {
            ...
            addedCategories.forEach((category) => {
               ...
               CategoryMangaController.addCategoryManga(tempCategoryManga).then((res) => {
               });
            });
            ...
            PublisherMangaController.addPublisherManga(tempPublisherManga).then((res) => {
    ...
    return (
        <div className="w-1/3 mt-8 p-6 bg-black bg-opacity-50 rounded">
            ...
            <div className="mb-4">
                <label htmlFor="status" className="block text-white text-sm font-medium">
                    Status
                </label>
                <select
                    id="status"
            ...
            <div className="mb-4">
                <label htmlFor="publishDate" className="block text-sm font-medium text-white">
                    Publish Date
                </label>
                <input
                    type="date"
            ...
            <div className="mb-4">
                ...
                {picture && (
                    <div className="mt-4">
                        <img src={picture} alt="Selected" className="w-full border-2 border-white rounded-xl" />
                    </div>
                )}
            </div>
            <div className="mb-4 w-full">
                <label htmlFor="authors" className="w-full text-sm font-medium text-white">
                    Authors
                </label>
                ...
                {addedAuthors.map((author) => (
                    <div key={author.aid} className="flex items-center justify-between bg-red-950 bg-opacity-50 text-white mt-2">
            ...
            <div className="mb-4 w-full">
                <label htmlFor="publishers" className="w-full text-sm font-medium text-white">
                    Publishers
                </label>
            ...
            <div className="mb-4 w-full">
                <label htmlFor="categories" className="w-full text-sm font-medium text-white">
                    Categories
                </label>
                <div className="flex items-center">
                    <select
                ...
                {addedCategories.map((category) => (
                    <div key={category.cid} className="flex items-center justify-between bg-red-950 bg-opacity-50 text-white mt-2">
            ...
            <button
                ...
                className={`w-full p-2 bg-blue-500 text-white rounded ${
                    !title || !status || !publishDate || !description || addedAuthors.length===0 || addedCategories.length===0 || !selectedPublisher ? 'opacity-50 cursor-not-allowed' : ''
                }`}
                disabled={!title || !status || !publishDate || !description || addedAuthors.length===0 || addedCategories.length===0 || !selectedPublisher}
            >
                Submit
            </button>
