...
function ChapterViewer() {
    ...
    return (
        <div className="background_image_home " style={volumeImage ? { backgroundImage: `url(${volumeImage.vcPicture})` } : { backgroundImage: `url("./../../Assets/Images/Backgrounds/home-page-bg.jpg")` }}>
            {manga && (
            ...
            <div className="bg-black bg-opacity-75 flex">
                <div className="bg-black bg-opacity-30 text-white">
                    {volumes.map((volume) => (
                        <div onClick={() => {setSelectedVolume(volume)}} key={volume.vid} className={` ${selectedVolume && volume.vid === selectedVolume.vid && "bg-gray-950 bg-opacity-75 font-bold"} hover:cursor-pointer hover:bg-black hover:bg-opacity-75 p-4 px-20  hover:border-gray-400 hover:scale-105 transition-transform duration-100 ease-in-out`}>
