...
const Pagination: React.FC<PaginationProps> = ({ totalItems, itemsPerPage, onPageChange }) => {
    ...
    const renderPageNumbers = () => {
        ...
        for (let i = startPage; i <= endPage; i++) {
            pageNumbers.push(
                <li
                    key={i}
                    className={`pagination-item ${i === currentPage ? 'bg-black text-white' : 'bg-black bg-opacity-75 text-gray-700'} hover:bg-opacity-90 hover:text-white cursor-pointer py-2 px-4 text-center hover:border-white border-2 border-gray-600 rounded`}
    ...
    return (
        <div className="flex items-center justify-center mt-8 pb-4">
            <button
                className={`pagination-control ${currentPage === 1 ? 'opacity-50 cursor-not-allowed' : ''} me-2 bg-black hover:bg-opacity-75 hover:border-white border-2 border-gray-600 text-white px-4 py-2 rounded`}
            ...
            <button
                className={`pagination-control ${currentPage === totalPages ? 'opacity-50 cursor-not-allowed' : ''} ms-2 bg-black hover:bg-opacity-75 hover:border-white border-2 border-gray-600 text-white px-4 py-2 rounded`}
                onClick={() => handlePageChange(currentPage + 1)}
