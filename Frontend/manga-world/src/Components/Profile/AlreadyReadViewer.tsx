...
function AlreadyReadViewer(props: AlreadyReadViewerProps) {
    ...
    useEffect(() => {
        if(userId){
            WishlistController.getAllAlreadyRead(userId, pageNumber-1).then((res) => {
            ...
            <div>
                {alreadyReads.map((alreadyRead, index) => (
                    ...
                    >
                        ...
                        <div onClick={() => {removeItem(alreadyRead)}} className="bg-red-700 p-3 m-1 rounded hover:scale-105 hover:cursor-pointer hover:bg-red-900 transition-transform duration-300 ease-in-out">
                            Remove
                        </div>
                    </div>
                ))}
            </div>
            <Pagination totalItems={totalItems} itemsPerPage={20} onPageChange={onPageChange}/>
        </div>
    );
}
